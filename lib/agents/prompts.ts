...
export const SECTION_GUIDANCE: Record<SectionKey, string> = {
  ...
  community_sentiment:
    "Objective: Synthesize employee reviews, customer reviews, and community discussion. Priority for `reviews`: (1) Glassdoor + AmbitionBox = employee/workplace reviews; (2) G2 + Capterra + Trustpilot = customer/product reviews. One entry per distinct site with source, rating, reviewCount, short summary, top 2-4 pros/cons, up to 3 sampleQuotes, and url. Prefer scraped review pages over salary/DEI side pages. Theme names must be short noun phrases (e.g. 'Pricing at scale', 'Deploy DX') — never full sentences. examples: 1-2 concrete snippets. overall + trend from recurring themes. Separate product issues from support/billing/culture. Include every on-topic review site found. If employee review sites are missing, still fill customer review sites; if both are thin, leave reviews=[] and set trend=unknown.",
  security_compliance:
    "Objective: Assess enterprise security readiness from evidence only. Priority: /security, /trust, /trust-center, compliance pages, and named certifications (SOC2, ISO27001, HIPAA, GDPR). Check certifications with status available/claimed/not_found/unknown. Check controls (SSO, SCIM, RBAC, encryption, audit logs) with supported/limited/not_found/unknown. enterpriseReadiness (0-100) reflects evidence of controls and certifications, not assumptions. List concerns and any documented incidents with citations.",
  pricing_intelligence:
    "Objective: Extract pricing model, published plans, and likely hidden costs from evidence. Priority: official /pricing|/plans pages, then third-party pricing directories, then complaint threads. model: seat, usage, hybrid, or null if undisclosed. plans: name, price, unit, notes — only plans explicitly mentioned. hiddenCosts: overages, minimums, support tiers, implementation fees found or strongly implied. competitorComparison: only competitors named in evidence with relativePrice and notes.",
  competitor_analysis:
    "Objective: Compare against real named competitors found in evidence — never invent competitors. Priority: G2 compare/alternatives pages, vendor comparison pages, and press that names rivals. For each: positioning, strengths, weaknesses with evidenceIds. featureMatrix: 4-8 comparison dimensions with yes/no/limited/unknown per competitor. Prefer competitors the vendor or third-party sources explicitly mention.",
  engineering_health:
    "Objective: Assess engineering quality signals visible from public evidence. Priority: /docs, /api, changelog, status page, SDK repos. Rate documentationQuality, apiQuality, sdkMaturity (excellent→unknown). Note releaseCadence, statusPage presence/URL, openSourceSignals, deprecationPolicy. notes: concrete observations (e.g. 'OpenAPI spec linked from docs', 'Last SDK release 8 months ago'). Do not score highly without supporting evidence.",
  risk_assessment:
